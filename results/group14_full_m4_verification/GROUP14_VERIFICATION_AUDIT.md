...
This is a direct, **no-fit** comparison using `m4_full`: M4 kinetic parameters are left unchanged, and the model is evaluated at the Group 14 temperature, pressure, feed, and 3 g catalyst mass. The plot compares the model prediction with the document's reported 99.207% CO conversion. That reported value is not reconciled with the same document's GC composition and flow measurements, so treat this as an unresolved reported-value comparison, not model validation.

| Metric | Full M4 prediction | Reported value | Prediction minus reported value |
|---|---:|---:|---:|
| CO conversion | 40.912% | 99.207% | -58.30 percentage points |

The model run completed: `True`. Model conversion is calculated from molar flows as (inlet CO - outlet CO) / inlet CO. Its predicted methane yield on the CO-inlet basis is 29.164%.
...
- Kinetics: Full M4 with published M4 parameters and activity fixed at 1.0. Full M4 includes direct CO2 methanation, CO methanation, and WGS (the reverse of the paper's RWGS reaction).
- Dry-inlet treatment: A leading-order inlet expansion starts full M4 from the documented zero-water, zero-CO2 CO/H2 feed without adding an artificial steam flow.
